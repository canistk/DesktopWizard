...
namespace WinOverlay
{
	...
	// Note: These methods are commented out as they require Windows.Forms types
	// which are not used in WPF. If you need to send events, create new methods
	// that construct KeyboardEventP3 and MouseEventP3 directly from WPF event data.
	...
	*/
	...
	public void SendKeyboard(KeyboardEventP3 evt)
	{
		var data = evt.ToByteArray();
		Task.Run(() => SendByte(data), m_CancelToken);
	}

	public void SendMouse(MouseEventP3 evt)
	{
		var data = evt.ToByteArray();
		Task.Run(() => SendByte(data), m_CancelToken);
	}
